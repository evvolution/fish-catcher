...
import { assertGapCopyPolicy, isGapCopyCandidateAllowed } from "./gap-content-policy.mjs";

const VERSION = `2026-07-18-semantic-v${GAP_SEMANTIC_VERSION}`;
const OUTPUT_URL = new URL("../src/lib/gap-copy-corpus.json", import.meta.url);
const QUOTE_SOURCE_URL = new URL("../src/lib/gap-quotes.ts", import.meta.url);
...
const entries = [
  ...
  return {
    ...
    semantics: analyzeGapSemantics(output),
  };
});

assert.equal(entries.length, 1000);
entries.forEach(assertGapCopyPolicy);
writeFileSync(
  OUTPUT_URL,
  `${JSON.stringify(
    {
...
function rankActivities(entry) {
  return ACTIVITIES.map((activitySlug) => {
    const semantics = analyzeGapSemantics({
    ...
    return {
      activitySlug,
      score: scoreGapSemanticActivityFit(semantics, activitySlug) * 10 + scoreActivitySurface(entry.content, activitySlug),
    };
  })
...
function takeUnique(entries, count) {
  ...
  for (const entry of entries) {
    if (!isGapCopyCandidateAllowed(entry)) continue;
