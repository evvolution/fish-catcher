...
import { assertMoyuCopyPolicy, findBlockedChinesePoliticalReference } from "./moyu-content-policy.mjs";

const curatedSource = readFileSync(new URL("../src/lib/moyu-quotes.ts", import.meta.url), "utf8");
const managedContentSource = readFileSync(new URL("../src/lib/moyu-content-seeds.ts", import.meta.url), "utf8");
const corpus = JSON.parse(readFileSync(new URL("../src/lib/moyu-copy-corpus.json", import.meta.url), "utf8"));
const forum = JSON.parse(readFileSync(new URL("../src/lib/moyu-forum-copy.json", import.meta.url), "utf8"));
...
const activityFit = corpus.entries.map((entry) => {
  const scores = ["drift", "tea", "breathe", "stroll"].map((activitySlug) =>
    scoreMoyuSemanticActivityFit(entry.semantics, activitySlug),
  );
  return Math.max(...scores) - scoreMoyuSemanticActivityFit(entry.semantics, entry.activitySlug);
});
...
for (const entry of corpus.entries) {
  ...
  assert.equal(entry.semantics?.version, MOYU_SEMANTIC_VERSION, `${entry.slug} semantic version mismatch`);
  assertTagList(entry, "scenes", moyuSemanticValues.scene, 1, 2);
  assertTagList(entry, "emotionalCores", moyuSemanticValues.emotionalCore, 1, 2);
  assert.ok(
    moyuSemanticValues.psychologicalNeed.includes(entry.semantics.psychologicalNeed),
    `${entry.slug} psychological need is invalid`,
  );
  assert.ok(
    moyuSemanticValues.literaryGesture.includes(entry.semantics.literaryGesture),
    `${entry.slug} literary gesture is invalid`,
  );
  assert.ok(moyuSemanticValues.energy.includes(entry.semantics.energy), `${entry.slug} energy is invalid`);
...
console.log("moyu quote library: 1200 unique entries; public-domain corpus and original forum layer are semantic, balanced, sourced, clean, fresh, and politically screened");
