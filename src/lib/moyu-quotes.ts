import { analyzeMoyuSemantics, semanticDimensionRefs } from "~~/src/lib/moyu-semantics";

export type MoyuQuoteSeed = {
  slug: string;
  kind: "RESULT" | "CARD";
  title: string;
  content: string;
...
export const moyuQuoteSeeds: MoyuQuoteSeed[] = [
...
function quote(
  ...
  title: string,
  content: string,
  activitySlug: MoyuQuoteSeed["activitySlug"],
  notes: string,
  ...
  const semantics = analyzeMoyuSemantics({ content, title, activitySlug });
